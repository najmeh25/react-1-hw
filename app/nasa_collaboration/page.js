...
const API_KEY = 'yfeXElzZbfcI8Op2vQh9PynOxURTXTzw8ML9vxOM';
...
const RoverPhoto = ({ src, date, roverName }) => (
  <div className={styles.roverPhotoCard}>
    ...
    <img className={styles.nasaPicOfTheDayImg}  src={src} alt={`Photo taken by ${roverName} on ${date}`} />
  </div>
);
...
const [roverPhoto, setRoverPhoto] = useState([]);
  ...
  useEffect(() => {
    const fetchRoverPhotos = async () => {
      try {
        const response = await fetch(`https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?earth_date=2015-06-03&api_key=${API_KEY}`);
